...
- **ツールチップのグループ別表示**: スタックグループが定義されている場合、ツールチップは payload をグループ単位にバケット化して表示する（`[TW60]` / `[PH50]` のヘッダ行付き）。未割り当て系列はヘッダなしのセクションに集約。`stackGroups` 未指定タイルでは単一バケットになりヘッダ行が出ないため従来とほぼ同じ見た目
- **ホバー中グループだけ表示**: 棒グラフのいずれかのセグメントにマウスポインタが乗ると、その系列が属する `stackGroupId` を内部 state にセットし、ツールチップではそのグループのバケットのみ表示する。グループ未割り当て系列・折れ線系列は常に表示される。バーの外に出ると state はクリアされ、全グループ表示に戻る
- **棒の上端のグループラベル**: 各 (stackGroup × yAxisId) ペアで最後に登場する Bar 系列（=積み上げの最上端）の上に、グループ名（例: `TW60`）が薄いグレーで描画される。`data` の各 point に `_stackTotal#{groupId}#{yAxisId}` という事前計算フィールドを付与し、それを LabelList の dataKey にすることで、最上端 Bar 自体が 0 のときでもスタック合計が 0 でなければラベルが表示される（=その日付のスタック全体が 0 の日のみラベル非表示）。件数ラベル表示と同時に有効な場合は offset を縮めて両方が読めるよう配置される。`stackGroups` 未定義時 / `barStackMode !== 'stacked'` 時は表示されない
