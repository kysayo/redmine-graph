...
- **棒グラフ積み上げ（`barStackMode`）**: タイル単位のフラグで `'grouped'`（省略時デフォルト=隣接配置）/ `'stacked'`（積み上げ）を切り替える。`'stacked'` のとき棒系列は **`yAxisId` × `stackGroupId` × 分割元系列（`_splitParentId`）** の組み合わせごとに独立して積み上げられる（左軸の棒と右軸の棒は混ざらず、スタックグループが違えば同じ軸内でも別の積み上げになり、さらに**分割キーで展開された系列は展開元の系列ごとに別の棒**になる）。これにより「Created を分割キーで積み上げた棒」と「Completed を分割キーで積み上げた棒」が同じ軸でも連結されず横並びになる。分割していない系列同士は従来通り1本に積み上がる（`_splitParentId` が無い系列は同一スタック扱い）。折れ線（`line`）系列は積み上げ非対象で従来通り重ね描画される。設定パネルの「グラフ表示設定」末尾の「棒グラフを積み上げ」チェックボックスで切り替え。OFF時は値が `undefined` で保存される（`'grouped'` は保存しない）。件数ラベル表示時は積み上げモードでは `position="insideTop"`（各セグメント上端内側）に切り替わり、各セグメントの値が個別に表示される
- **系列の分割キー（`splitBy`）**: 1つの系列を、指定フィールドの**値ごとに自動的に複数系列へ複製**する機能。`barStackMode: 'stacked'` と併用することで「カテゴリごとに積み上がった1本の棒」を、系列を手で並べずに描画できる。展開は取得済みチケット（`issueState.issues`）に実際に現れた値から行うため、**プロジェクトやカテゴリが増えても設定変更が不要**
  - **値は ID ではなく「表示名」で束ねられる**。複数プロジェクトを横断表示していて同名のカテゴリがプロジェクトごとに別IDになっている場合（例: 4プロジェクトそれぞれの「Finance」カテゴリ）でも、1つの系列（1つのセグメント）に統合される。内部的には `name:{field}` 形式の仮想フィールド条件（`issueAggregator.conditionMatchesIssue`）で判定する
  - **分割元の系列ごとに別の積み上げ棒になる**。展開後の系列は `_splitParentId`（展開元の系列ID）を持ち、`stackId` にこれが含まれるため、同じ軸に「Created を分割した棒」と「Completed を分割した棒」があっても連結されず横並びで表示される。各棒の上端には分割元の系列名（例: `Created`）が見出しとして描画される
  - 展開後の系列 id は `${origId}~sp{index}` 形式。ラベルは分割値の名前、色は `COLOR_PALETTE` からタイル内通しで自動割り当て（**元系列の系列名・色設定は使われない**。系列名は棒の上端の見出しと、値名が衝突したときの一意化にのみ使われる）。同じ値名を複数の系列が生成する場合のみ ` (系列名)` を付けて一意化し、凡例の重複排除で潰れないようにする
  - **並び順（`splitOrder`）**: `'count'`（件数降順・省略時デフォルト）/ `'name'`（名前昇順）。積み上げの下から順にこの順序で並ぶ。**上限（`splitLimit`）**: 表示する分割値の上限件数（省略時 = 全件）
  - 分割値の抽出時にも「タイル共通条件 ∧ 系列の絞り込み条件 ∧ 対象ステータス」が適用されるため、条件に合致するチケットが 0 件の値は系列として生成されない
  - 値が未設定（`null`/空文字）のチケットはスキップされ、どのセグメントにも計上されない
  - `difference` / `sum` 系列は参照関係（`refSeriesIds`）が壊れるため分割対象外。逆に分割キー付き系列は `difference`/`sum` の参照候補にも現れない
  - Redmine に接続できずダミーデータを表示している場合は展開されず、元の 1 系列のまま描画される
  - 展開は `App.tsx` の `combosResolvedSeries`（`expandSeriesBySplit()`）で**集計前に一度だけ**行い、同じ配列を `aggregateIssues()` と `ComboChart` の両方に渡す。したがって `stackGroups` による展開（`@{groupId}`）とも併用でき、その場合は 分割値 × スタックグループ の組み合わせ数だけ仮想系列が生成される
- **スタックグループ（`stackGroups`）**: 同一タイル内に複数の独立した積み上げ棒を**横並び**で表示するための単位。`ComboChartConfig.stackGroups: ComboStackGroupConfig[]` でグループを定義する（各グループは `id` / `label` / `commonConditions?` を持つ）。**系列はグループの数だけ自動的に複製・展開される**：N 系列 × M グループのとき、内部的に N×M 個の仮想系列が生成され、それぞれの `dataKey` は `${origSeriesId}@${groupId}` 形式になる。同じく `stackId` は `${yAxisId}-${groupId}` で同一軸内のグループ間で別の積み上げ棒を作る（`stackGroups` 未定義時は `_default` スタックで従来挙動）。各グループの `commonConditions` は所属系列の集計に **AND** で適用され、タイル全体の `commonConditions` ともさらに AND される。`cumulative` / `difference` / `sum` 系列もグループごとに独立して展開される（`refSeriesIds` は同じグループ内の仮想系列を指すよう自動マッピング）。ユーザーが系列を 1 セット定義するだけで、グループの数だけ並んだ積み上げ棒が描画される
- **凡例の重複排除**: 同一 `label` を持つ系列が複数（例: スタックグループ TW60 と PH50 の両方に「Tier1ナレッジ不足」がある）ある場合、凡例には**最初に登場した1個だけ**が表示される。代表色は最初の系列の色を使うため、同名系列は同じ色で運用する想定（色は `GraphSettingsPanel` の系列カードで個別に設定するため、ユーザーが手動で揃える）
- **ツールチップの積み上げ棒別表示**: スタックグループまたは分割キーが使われている場合、ツールチップは payload を積み上げ棒（バケットID = `${stackGroupId}|${splitParentId}`）単位にバケット化して表示する（`[TW60]` / `[Created]` / `[TW60 / Created]` のヘッダ行付き）。どちらにも属さない系列はヘッダなしのセクションに集約。両方とも未使用のタイルでは単一バケットになりヘッダ行が出ないため従来とほぼ同じ見た目
- **ホバー中の棒だけ表示**: 棒グラフのいずれかのセグメントにマウスポインタが乗ると、そのバケットIDを内部 state にセットし、ツールチップではそのバケットのみ表示する。未割り当て系列・折れ線系列は常に表示される。バーの外に出ると state はクリアされ、全バケット表示に戻る
- **棒の上端の見出しラベル**: 各積み上げ単位（`yAxisId` × `stackGroupId` × 分割元系列）で最後に登場する Bar 系列（=積み上げの最上端）の上に、見出し（スタックグループ名 / 分割元の系列名。両方あれば `TW60 / Created` のように併記）が薄いグレーで描画される。`data` の各 point に `_stackTotal#{stackKey}` という事前計算フィールドを付与し、それを LabelList の dataKey にすることで、最上端 Bar 自体が 0 のときでもスタック合計が 0 でなければラベルが表示される（=その日付のスタック全体が 0 の日のみラベル非表示）。件数ラベル表示と同時に有効な場合は offset を縮めて両方が読めるよう配置される。スタックグループ・分割キーのどちらも未使用時 / `barStackMode !== 'stacked'` 時は表示されない
...
- **グラフ表示設定**（全系列共通）:
  ...
  - 棒グラフを積み上げ: チェック時は棒グラフ系列を `yAxisId` × `stackGroupId` ごとに独立して積み上げ表示（左軸棒・右軸棒・別グループ棒は混ざらない）。折れ線（line）系列は積み上げ非対象で従来通り重ね描画。OFF=`undefined`/ON=`'stacked'` を `ComboChartConfig.barStackMode` に保存。件数表示 ON 時は積み上げモードでは `position="insideTop"` でセグメント上端内側に値が表示される
  ...
  - **スタックグループ**: 共通絞り込み条件の下、系列カードの上に折り畳みセクションとして表示。「＋ スタックグループを追加」で `ComboStackGroupConfig`（`{ id, label, commonConditions? }`）を任意個数追加でき、各グループにラベル（例: `TW60`）と専用の共通絞り込み条件（`ConditionsEditor`）を設定する。各グループ行には ↑↓・削除ボタンを配置。**系列はグループの数だけ自動的に複製・集計される**ため、ユーザーが系列を複数セット定義する必要はない（系列カードには「グループ」プルダウン無し）。集計時は タイル `commonConditions` ∧ グループ `commonConditions` ∧ 系列 `conditions` の **AND** がチケットフィルタとして適用される。同名系列がグループ数だけ生成されても、凡例は系列名でユニーク化されて 1 回だけ表示される。折り畳み状態は `collapsedCards` に `combo-{id}-stackgroups` キーで永続化される。スキーマ拡張のみで設定 JSON / プリセット / TeamPreset に自動追従する
...
- 各系列に設定できる項目:
  ...
  - 絞り込み条件（系列ごと）: チケットの項目で絞り込み。フィールド（react-select、テキスト入力補完あり）・演算子・値の組み合わせ。複数条件はAND。`ComboChartConfig.commonConditions`（共通絞り込み条件）が設定されている場合、それと **AND** で結合される。対応フィールド: ステータス、トラッカー、優先度、カスタムフィールド（リスト系・テキスト型）、経過日数（日）、**日付フィールド**（開始日・期日・カスタム日付フィールドなど `type === 'date'` のフィールド）。ページリロード後も復元のため、マウント時に設定済みフィールドの選択肢を事前取得する（日付・経過日数・テキスト型フィールドはAPIフェッチ不要）
    - **日付フィールド絞り込み**: 日付型フィールドを選択すると専用UIに切り替わる。演算子は「記入済み / 空（未設定） / より前（<）/ 以前（<=）/ より後（>）/ 以降（>=）/ 今週 / 来週 / 先週 / 今週まで / 来週まで / 来週以降 / 今月」から選択。`<`/`<=`/`>`/`>=` 選択時は「今日」チェックボックスまたは固定日付ピッカーで比較値を指定。`SeriesCondition.dateCondition` フィールドに格納（`values` は使用しない）。日付型フィールドでは演算子セレクトを非表示にし、`dateCondition` 内で演算子を管理
  ...
  - 分割キー: 選択したフィールドの値ごとに系列を自動複製する（react-select、リスト系フィールドのみ）。未選択 = 分割なし。選択すると並び順セレクト（件数降順／名前昇順）と上限件数の入力欄が現れる。分割キー指定時は系列名・色の設定が無視され、凡例には分割値の名前が表示される。`aggregation` が `difference`/`sum` の系列では非表示（選択済みの状態で `difference`/`sum` に切り替えると自動的にクリアされる）
...
- **条件フィルタ**: `conditions[]` に設定された絞り込み条件でチケットをフィルタ（AND条件）。対応フィールド: `status_id`・`subproject_id`/`project_id`（`issue.project.id` と比較）・`tracker_id`・`priority_id`・`author_id`・`assigned_to_id`・`category_id`・`fixed_version_id`・`cf_{id}`（カスタムフィールド：リスト系・テキスト型の両方）・`elapsed_days`（経過日数、仮想フィールド）。演算子: `=`（一致）、`!`（不一致）、`>=`（以上）、`<=`（以内）、`~`（含む／部分一致、case-insensitive）、`!~`（含まない）
  ...
  - **`name:{field}` 仮想フィールド**: フィールドキーが `name:` で始まる条件は、ID ではなく `getIssueGroupValue()` が返す**表示名**と比較する（例: `name:category_id` = `"Finance"`）。複数プロジェクトを横断表示していて同名のカテゴリ・バージョン等がプロジェクトごとに別IDになっているケースを1つにまとめるためのもので、系列の分割キー（`splitBy`）が内部的に生成する。Redmine の URL フィルタには変換できないため、設定UIの項目選択肢には出さない（内部専用）
- **系列の分割展開**: `expandSeriesBySplit(series, issues, commonConditions?)` は `splitBy` が設定された系列を、`resolveSplitValues()` が返す値の数だけ複製した系列配列を返す。`resolveSplitValues(issues, splitBy, conditions?, statusIds?, order?, limit?)` は条件に合致するチケットに現れた値名を件数降順（または名前昇順）で列挙する。展開済み系列には `name:{splitBy} = 値` の条件が AND で追加される
...
| フィールド | 型 | 説明 |
|---|---|---|
| `field` | `string` | `availableFilters` のキー（例: `cf_628`, `tracker_id`, `elapsed_days`）。`name:` プレフィックス付き（例: `name:category_id`）の場合は ID ではなく表示名で比較する仮想フィールド（系列の `splitBy` が内部生成） |
...
| フィールド | 型 | 説明 |
|---|---|---|
| `op` | `'empty' \| 'not_empty' \| '<' \| '<=' \| '>' \| '>=' \| 'this_week' \| 'next_week' \| 'last_week' \| 'to_this_week' \| 'to_next_week' \| 'from_next_week' \| 'this_month'` | 比較演算子。`empty`=未設定、`not_empty`=設定あり、その他=日付文字列の辞書順比較。週キーワード: `this_week`/`next_week`/`last_week`=週内（月曜〜日曜）、`to_this_week`=今週まで（〜今週日曜）、`to_next_week`=来週まで（〜来週日曜）、`from_next_week`=来週以降（来週月曜〜）。`this_month`=今月内（月初〜月末、JST基準）。いずれもJST基準 |
...
- **グルーピング（`rowGroupRules` / `colGroupRules`）**: 複数の値を1行/列にまとめるルールを定義可能（`PieGroupRulesEditor` を再利用）
  ...
  - **日付条件グルーピング（`dateCondition`）**: ルールに `dateCondition` を設定すると `values` の代わりに日付比較で集計する。演算子: `empty`（未設定）/ `not_empty`（設定あり）/ `<`（より前）/ `<=`（以前）/ `>`（より後）/ `>=`（以降）/ `this_week`（今週月〜日）/ `next_week`（来週月〜日）/ `last_week`（先週月〜日）/ `to_this_week`（今週まで）/ `to_next_week`（来週まで）/ `from_next_week`（来週以降）/ `this_month`（今月内、月初〜月末）。`value: 'today'` で実行時の今日JST日付と比較。クリック時のURLフィルタ: 単方向演算子は `<=`/`>=` に変換、週内キーワード（`this_week` 等）・`this_month` は Redmine の `><`（between）演算子を用いた期間指定に変換、`to_*`/`from_*` は `<=`/`>=` に変換される
...
- **数値列の空欄は正常値**: 数値列（`type: 'number'`）の空欄は「値なし」を意味する正常な状態として扱う（料金無償で参加する BK メンバーなど、リソース数を持たない担当者を空欄で表現するため）。空欄以外で数値として解釈できない値のみをエラーとして扱う（`utils/extraValues.ts`）
- **数値列を計算（割り算など）に使う場合の方針**: `toNumericCellValue()` で数値化し、`null`（空欄または数値以外）が返ったときは**計算せずセルだけをエラー表示**（`NUMERIC_CELL_ERROR_TEXT` = `—` を `NUMERIC_CELL_ERROR_STYLE` の赤字で表示）にして、表全体の描画は継続する。1人分の値が不正でも表が出なくなることのないようにする
...
- **数値列の検証**: 取り込み後に数値列（`type: 'number'` = Resource 等）の値を検査し、数値以外が入っていれば設定パネルに黄色の警告バーを表示する（`collectNumericCellIssues` / `formatNumericCellIssues`）。**取り込み自体は中断せず完了する**（値はそのまま保持される）。警告は × で閉じられる
...
- **数値列の検証**: Resource 等の数値列に数値以外が入っている場合、取り込みは完了したうえで設定パネルに黄色の警告バーを表示する。空欄は正常値として警告しない（AssignmentMappingPanel と同一仕様）
- データ行が無いCSV（1行のみ）を読み込んだ場合も警告バーで通知する
...
- ヘッダーに「更新」「設定」「削除」ボタン。**データ収集日**（ソースチケットの `updated_on` = ジャーナル収集タイルが最後に保存した日）を表示。3日以上前の場合は警告色（`#b45309`）で表示する
- **「更新」ボタンはソースチケットのJSON再取得のみ**を行う。タイルのマウント時（ページ読み込み時）と `sourceIssueId` / `apiKey` 変更時に同じ取得が自動実行されるため、通常は押す必要がない（ページを開いたまま別タブで収集を実行した場合などに使う）
- **集計値は常に最新**: 担当者・集計範囲・週単位表示月・トラッカーフィルタの変更は `useMemo` により即時再計算される（「更新」ボタン不要）
