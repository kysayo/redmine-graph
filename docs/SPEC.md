...
- **compact モード**: `tileColumns` が 1 または 2 のとき自動でコンパクト表示に切り替わる（セルパディング縮小・列ヘッダーテキスト折り返し許可・フォントサイズ 12px）。それでも幅が足りない場合は横スクロール（`overflowX: auto`）にフォールバック
