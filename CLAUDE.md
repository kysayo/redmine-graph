...
```
npm run build   # 動作確認のみ
git restore dist/moca-react-graph.iife.js   # dist を元に戻す
git add src/...   # src のみステージング
git commit & git push
```

`dist/moca-react-graph.iife.js` はjsDelivr のために追跡対象とする必要がある。ビルド確認をした時にはコミット前に dist を元に戻す必要がある。
...
(function() {
  function insertGraph() {
    ...
    script.src = 'https://cdn.jsdelivr.net/gh/kysayo/redmine-graph@a90f907/dist/moca-react-graph.iife.js';
