...
(function() {
  // ▼ カスタマイズ設定 ▼
  var manualUrl = ''; // Manualリンクの URL（空文字の場合はリンクを表示しない）
  // ▲ カスタマイズ設定 ▲
  ...
  function insertGraph() {
    ...
    // Manualリンク（manualUrl が設定されている場合のみ表示）
    if (manualUrl) {
      ...
      manualLink.style.cssText = 'font-size:12px; margin-left:12px; color:#4a9eda; text-decoration:underline; font-weight:normal; vertical-align:middle;';
      manualLink.setAttribute('onclick', 'event.stopPropagation();'); // legendのトグルを発火させない
      legend.appendChild(manualLink);
    }
...
| `data-holidays-issue-id` | 数値文字列（チケットID）          | `""`         | 祝日リストを記録したチケットのID。descriptionに `["2026-3-20","2026-4-29"]` 形式のJSON配列を記入。経過営業日数の計算で祝日がスキップされる |
